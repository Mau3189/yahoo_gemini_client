...
module YahooGeminiClient
  class Client
    attr_accessor(:consumer_key, :consumer_secret, :oauth2_client, :token)
    ...
    def get_token(authorization_code)
      self.token = oauth2_client.auth_code.get_token(
        authorization_code,
        redirect_uri: 'oob',
    ...
    def api_request_headers
      { 'Authorization' => "Bearer #{token.token}", 'User-Agent' => user_agent }
    end
    ...
    def token_refresh!
      ...
      new_token = token.refresh!(redirect_uri: 'oob', headers: oauth2_headers)
